...
<?php
	...
	$arrival = '';
	$charter = '';
	$sql = "select * from pireps where pirep_id='$flightid'";
	...
	{
		$sql = "delete from pireps where pirep_id='$flightid'";
		if (!$result = $db->query($sql)) {
			die('There was an error running the query [' . $db->error . ']');
		}
	}
	...
	{
		$sql = "update pireps set updated_at=NOW(), validator_comments='$comment' where pirep_id='$flightid'";
	...
	if ($action=="acceptflight")
	{
		...
		if ($paid!=1)
		{
			...
			$sql = "insert into va_finances (amount,parameter_id,finance_date,gvauser_id,description,report_type,report_id) values ($quantity, '99995',now(),$pilot ,'$fligth_type:$departure - $arrival','Manual report', '$flightid')";
			if (!$result = $db->query($sql)) {
				die('There was an error running the query [' . $db->error . ']');
			}
		}
		$sql = "update pireps set updated_at=NOW(), validator_comments='$comment', valid='1', paid='1' where pirep_id='$flightid'";
	...
	{
		$sql = "update pireps set updated_at=NOW(), validator_comments='$comment', valid=2   where pirep_id='$flightid'";
