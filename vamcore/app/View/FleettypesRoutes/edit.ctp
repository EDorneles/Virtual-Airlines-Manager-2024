...
<?php echo $this->Form->create('FleettypesRoute'); ?>
	<fieldset>
		<legend><?php echo __('Edit Fleet Type Route'); ?></legend>
	<?php
		echo $this->Form->input('id');
		echo $this->Form->input('route_id',['label' => 'Flight']);
		echo $this->Form->input('fleettype_id',['label' => 'Fleet type']);
	?>
	</fieldset>
...
<div class="actions">
	...
	<ul class="list-inline">
		...
		<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('FleettypesRoute.id')), array('class' => 'btn btn-md btn-danger'), __('Are you sure you want to delete # %s?', $this->Form->value('FleettypesRoute.id'))); ?></li>
	</ul>
</div>
<?php
    }
