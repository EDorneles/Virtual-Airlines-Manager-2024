...
<?php echo $this->Form->create('Fleettype'); ?>
	<fieldset>
		<legend><?php echo __('Edit Fleet type'); ?></legend>
	<?php
		echo $this->Form->input('fleettype_id');
		echo $this->Form->input('plane_icao',['label' => 'Aircraft ICAO']);
		echo $this->Form->input('plane_description',['label' => 'Aircraft description']);
...
<div class="actions">
	...
	<ul class="list-inline">
		...
		<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('Fleettype.fleettype_id')), array('class' => 'btn btn-md btn-danger'), __('Are you sure you want to delete # %s?', $this->Form->value('Fleettype.plane_icao'))); ?></li>
	</ul>
</div>
<?php
    }
